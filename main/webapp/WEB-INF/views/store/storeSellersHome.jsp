...
<head>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/storeJS/storeSellersHome.js"></script>
        ...
        <main id="seller-home">
            ...
            <div id="seller-home-list">
                ...
                <div id="seller-home-listnav">
                    <button onclick="order(`${si.sellerPageNo}`, 0)" class="list-order" value="0" style="border-left: 0;">인기도순</button>
                    <button onclick="order(`${si.sellerPageNo}`, 1)" class="list-order" value="1">최신등록순</button>
                    <button onclick="order(`${si.sellerPageNo}`, 2)" class="list-order" value="2">낮은 가격순</button>
                    <button onclick="order(`${si.sellerPageNo}`, 3)" class="list-order" value="3">높은 가격순</button>
                    <button onclick="order(`${si.sellerPageNo}`, 4)" class="list-order" value="4" style="border-right: 0;">리뷰 많은순</button>
                </div>
                    ...
                    <div id="pagination-div">
                        <ul class="pagination">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link" href="sellersStore.st?sellerPageNo=${si.sellerPageNo}&order=${order}&cpage=${pi.currentPage - 1}">&laquo;</a></li>
                                </c:otherwise>
                            </c:choose>
                            <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
                                <li class="page-item ${p == pi.currentPage ? 'active' : ''}"><a class="page-link" href="sellersStore.st?sellerPageNo=${si.sellerPageNo}&order=${order}&cpage=${p}">${p}</a></li>
                            </c:forEach>
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link" href="sellersStore.st?sellerPageNo=${si.sellerPageNo}&order=${order}&cpage=${pi.currentPage + 1}">&raquo;</a></li>
                                </c:otherwise>
                            </c:choose>
                        </ul>
                    </div>
